...
const statusStyles = {
  ...
  upcoming: 'border-cyan-400/30 bg-cyan-400/15 text-cyan-300',
  soon: 'border-[#ff9f1c]/35 bg-[#ff9f1c]/15 text-[#ffbf69]',
  finished: 'border-slate-600 bg-slate-700/30 text-slate-300'
}

const actionTo = (t) => t.status === 'active' ? `/bracket/${t.id}` : '/tournaments'
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="mb-1 text-xs text-slate-500">
              {t.status === 'soon' ? 'Inicia en 2 dias' : `${t.status === 'active' ? 'Ronda 2/4' : `${t.participants}/${t.totalSlots} slots`}`}
            </div>
